...
void vk_pipelines_init(struct vk_renderer *renderer, struct vk_render_setup *setup,
	...
	*out_pipelines = (struct vk_pipelines) {0};
	...
	create_vk_quad_pipelines(renderer, setup, &renderer->shader_info.quad,
			&out_pipelines->quad);
	create_vk_quad_pipelines(renderer, setup, &renderer->shader_info.box_shadow,
			&out_pipelines->box_shadow);
...
void vk_pipelines_destroy(struct vk_renderer *renderer, struct vk_pipelines *pipelines) {
	...
	delete_vk_quad_pipelines(renderer, pipelines->box_shadow);
